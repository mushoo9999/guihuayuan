    ...
    <table class="Mytable" border="0" cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                <th>序号1</th>
                <th>方案名称</th>
                <th v-for="(item,index) in inputdata[0].resList" :key="item">     {{item.resName}}起调水位(m)
        ...
        <tbody class="ivu-table-tbody">
            <tr v-for="(item,index) in inputdata" :key="item">
               ...
               <!--  <td>
                   <a class="del" v-on:click="myModels.modal2=true">泄流曲线</a>
                </td> -->
                <td>
    ...
    methods: {
        AddData() {
            ...
            that.inputdata.push(JSON.parse(JSON.stringify(that.inputdata[0])));
              that.inputdata[that.inputdata.length - 1].hyScheme.schemeCode="方案_"+that.inputdata.length;
